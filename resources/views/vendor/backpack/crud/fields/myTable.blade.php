        ...
        <table class="table table-sm table-striped m-b-0">
            ...
            <tbody class="table-striped items sortableOptions">
                ...
                <tr class="array-row clonable" style="display: none;">
                    @foreach( $field['columns'] as $subfield)
    @php
    $subfield['label']='';
    $subfield['name']=$field['name'].'.'.$subfield['name'].'[]';
    $subfield['fake']=true;
    $subfield['store_in']=$field['name'];
    @endphp
                    <td style="vertical-align: middle">
                        @include($crud->getFirstFieldView($subfield['type'], $subfield['view_namespace'] ?? false), ['field' => $subfield])
                    </td>
                    @endforeach
                    <td>
